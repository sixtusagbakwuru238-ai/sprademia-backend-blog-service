...
async function connect(): Promise<void> {
  try {
    ...
    await publishChannel.assertExchange(config.RABBITMQ_EXCHANGE, "topic", { durable: true });
    await consumeChannel.assertExchange(config.RABBITMQ_EXCHANGE, "topic", { durable: true });
...
export async function publish<T>(eventType: BlogEventType, payload: T): Promise<void> {
  if (!publishChannel) {
...
export async function subscribe(
  ...
  const ch = consumeChannel; // capture non-null for use in async callback
  ...
  await ch.bindQueue(queueName, config.RABBITMQ_EXCHANGE, routingPattern);
